    ...
    <section className="container pb-[150px]">
      <div className="mx-auto grid grid-cols-1 items-center justify-center gap-8 sm:grid-cols-2 lg:grid-cols-3">
        {userManageCard?.map((userCard) => (
          <div
            key={userCard.id}
            className="rounded-medium shadow-nav dark:bg-dark-200 relative max-w-[402px] border border-solid border-transparent bg-white p-8 transition-colors hover:transition-colors max-lg:p-5 hover:dark:border-gray-600">
            <figure>
              <Image
                src={userCard.img}
            ...
            <figcaption>
              <h3 className="mt-10 mb-2.5 max-w-[70%] leading-8">{userCard.title}</h3>
